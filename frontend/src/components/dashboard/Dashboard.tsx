...
const StatCard = ({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      // Simple animation: just set the value. For more complex animations,
      // you could use a library like react-spring or framer-motion.
      ...
      <div className="card-glass text-center py-16 animate-pulse">
        ...
        <div className="mt-6 space-y-2">
          <div className="h-2 bg-white/20 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full loading-shimmer"></div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {currentFlow !== 'dashboard' && (
        <div className="card-glass animate-slide-in-right">
            ...
            <div className="flex items-center space-x-6 mb-4 md:mb-0">
              {['upload', 'register', 'verify'].map((flow, index) => (
                <React.Fragment key={flow}>
                  ...
                  {index < 2 && <div className="h-px w-8 bg-white/20"></div>}
                </React.Fragment>
              ))}
            </div>
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <StatCard
              ...
              icon={<svg className="w-6 h-6 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>}
              delay={100}
            />
            <StatCard
              title="$HOMED Balance"
          ...
          <div className="card-glass animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <button onClick={() => setCurrentFlow('property-lookup')} className="interactive-card glass-dark rounded-2xl p-6 text-left magnetic-hover group">
                <div className="text-lg font-bold text-white mb-2 group-hover:text-purple-300">🔍 Property Lookup</div>
                <div className="text-sm text-gray-400">Search real property data</div>
              </button>
              <button onClick={() => setCurrentFlow('upload')} className="interactive-card glass-dark rounded-2xl p-6 text-left magnetic-hover group">
                <div className="text-lg font-bold text-white mb-2 group-hover:text-blue-300">🏠 Register Property</div>
                <div className="text-sm text-gray-400">Upload docs & register</div>
              </button>
              <button onClick={() => { setTokenToMintFrom(null); setCurrentFlow('mint-homed'); }} className="interactive-card glass-dark rounded-2xl p-6 text-left magnetic-hover group">
                <div className="text-lg font-bold text-white mb-2 group-hover:text-green-300">💰 Mint $HOMED</div>
                <div className="text-sm text-gray-400">Generate stablecoins</div>
              </button>
              <button onClick={() => setCurrentFlow('repay')} className="interactive-card glass-dark rounded-2xl p-6 text-left magnetic-hover group">
                <div className="text-lg font-bold text-white mb-2 group-hover:text-orange-300">📊 Repay Loan</div>
                <div className="text-sm text-gray-400">Repay your loan</div>
              </button>
            </div>
          </div>
            ...
            {properties.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {properties.map((property, index) => (
                  <div key={property.id} className="glass rounded-2xl p-6 border border-white/20 hover:border-white/40 transition-all duration-300 transform hover:scale-105" style={{ animationDelay: `${0.1 * index}s` }}>
                    <div className="flex items-start justify-between mb-4">
                      <h3 className="font-semibold text-white text-lg mb-2">{property.title}</h3>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${property.verificationStatus === "verified" ? "bg-green-500/20 text-green-400" : property.verificationStatus === "rejected" ? "bg-red-500/20 text-red-400" : "bg-yellow-500/20 text-yellow-400"}`}>{property.verificationStatus}</span>
                    </div>
                    ...
                    <button
                      ...
                      className="mt-4 w-full bg-green-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-green-700 disabled:opacity-50 transition-colors"
                    >
                      Mint $HOMED
                    </button>
                  </div>
              ...
              <div className="text-center py-16">
                <div className="w-24 h-24 mx-auto mb-6 glass rounded-3xl flex items-center justify-center"><svg className="w-12 h-12 text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div>
                <h3 className="text-xl font-semibold text-white mb-2">No Properties Yet</h3>
                <p className="text-white/70 mb-6">Get started by registering your first property</p>
                <button onClick={() => setCurrentFlow("upload")} className="btn-primary">Register Your First Property</button>
              </div>
            )}
          </div>
        </div>
