...
const StatCard = ({ title, value, icon, delay = 0 }: { title: string; value: string | number | bigint; icon: React.ReactNode; delay?: number; }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      else if (typeof value === 'string') { targetValue = parseFloat(value.replace(/,/g, '')) || 0; }
...
export const Dashboard: React.FC<{ onDisconnect: () => void }> = ({ onDisconnect }) => {
  ...
  const handleRegistrationComplete = (propertyId: string) => { if (propertyId === 'flow_complete') { handleBackToDashboard(); fetchDashboardData(); } else { setCurrentPropertyId(propertyId); setCurrentFlow('verify'); fetchDashboardData(); } };
  const handleVerificationComplete = (result: VerificationResult) => { if (result.finalApproval) { handleBackToDashboard(); fetchDashboardData(); } };
  const handleInitiateMint = (property: PropertyData) => { const numericId = parseInt(property.id, 10); if (!isNaN(numericId)) { setTokenToMintFrom(numericId); } else { console.error("Could not determine token ID from property:", property); setTokenToMintFrom(null); } setCurrentFlow('mint-homed'); };
  const handleMintSuccess = () => { fetchDashboardData(); setCurrentFlow('dashboard'); setTokenToMintFrom(null); };
  const handleMintError = (error: string) => { console.error('Minting error:', error); };
  ...
  return (
    <>
      ...
      <MintPopup isOpen={showMintPopup} onClose={() => { setCurrentFlow('dashboard'); setTokenToMintFrom(null); }} onMintSuccess={handleMintSuccess} onMintError={handleMintError} preSelectedTokenId={tokenToMintFrom} />
    </>
  );
};